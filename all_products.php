<?php
    ...
    $total_records = $con->query("SELECT `product_id` FROM `products`")->num_rows;
    ...
    $sql_product = "SELECT `product_id`, `product_name`, `product_price`, `product_image_path` FROM `products`". $sortby ." LIMIT $start_from,$num_per_page;";
	...
	<div class="banner">
        ...
        <div class="w3l_banner_nav_right_banner3" style="background: url(assets/images/3.jpg) no-repeat 0px 0px;
			...
			<div class="w3ls_w3l_banner_nav_right_grid w3ls_w3l_banner_nav_right_grid_sub">
				<h3 class="w3l_fruit">Tất cả sản phẩm</h3>
				<?php
					  ...
					  <div class="col-md-12">
						<nav>
							<ul class="pagination">
								<li <?php if ($page == 1) {
									echo 'class="disabled"';
								}?>><a href="all-products?page=<?php echo $page-1;?><?php if (isset($_GET['sortby'])) { echo '&sortby='.$sortby_num; } ?><?php if (isset($_GET['num_per_page'])) { echo '&num_per_page='.$num_per_page; } ?>" aria-label="Previous"><span aria-hidden="true">«</span></a></li>

								<?php
								for ($i=1; $i <= $total_pages; $i++) { 
								?>
								<li <?php if ($i == $page) { echo 'class="active"';} ?>>
									<a href="all-products?page=<?php echo $i;?><?php if (isset($_GET['sortby'])) { echo '&sortby='.$sortby_num; } ?><?php if (isset($_GET['num_per_page'])) { echo '&num_per_page='.$num_per_page; } ?>"><?php echo $i;?></a>
								</li>
								<?php } ?>
								
								<li <?php if ($page == $total_pages) {
									echo 'class="disabled"';
								}?>><a href="all-products?page=<?php echo $page+1;?><?php if (isset($_GET['sortby'])) { echo '&sortby='.$sortby_num; } ?><?php if (isset($_GET['num_per_page'])) { echo '&num_per_page='.$num_per_page; } ?>" aria-label="Next"><span aria-hidden="true">»</span></a></li>
							</ul>
						</nav>
					  </div>
				</div>
							...
							<div class="card">
								<div class="card-body cart">
									<div class="col-sm-12 empty-cart-cls text-center">
										...
										<h3 style="margin-bottom: 1.5rem;">Không có sản phẩm</h3>
